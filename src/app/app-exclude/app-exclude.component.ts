...
@Component({
  selector: 'app-app-exclude',
  templateUrl: './app-exclude.component.html',
  styleUrls: ['./app-exclude.component.scss']
})
export class AppExcludeComponent implements OnInit {
  ...
  constructor(
    ...
    fileService.newProEmmited$.subscribe(
      res => {
        // console.log(res);
        if (res === true) {
          const popLayer = document.getElementById('popLayer-exclude');
  ...
  getProjects(projects: string[]): void {
    ...
    if (projects.length === 0) {
      ...
      const popLayer = document.getElementById('popLayer-exclude');
      popLayer.style.display = 'block';
      const popBox = document.getElementById('OpenProject-exclude');
  ...
  open(): void {
    ...
    this.close('OpenProject-exclude');
  }

  close(id: string): void {
    // console.log(id)
    document.getElementById(id).style.display = 'none';
    document.getElementById('popLayer-exclude').style.display = 'none';
  }
